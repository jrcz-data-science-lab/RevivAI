				...
				<div className="flex justify-center font-bold pt-8 -mt-8">
					{'Documentation'.split('').map((letter, index) => (
						<motion.span
							...
							transition={{ duration: 0.4, delay: 0.8 + index * 0.02, type: 'spring', bounce: 0.5 }}
						>
							{letter}
						</motion.span>
					))}
				...
				<div className="flex justify-center italic">
					{'from'.split('').map((letter, index) => (
						<motion.span
							...
							initial={{ translateY: 80, rotate: 60 }}
							animate={{ translateY: 0, rotate: 0 }}
							transition={{ duration: 0.5, delay: 1 + index * 0.04, type: 'spring', bounce: 0.55 }}
						>
							{letter}
						</motion.span>
					))}
				...
				<div className="flex justify-center font-bold">
					{'Code.'.split('').map((letter, index) => (
						<motion.span
							...
							animate={{ translateY: 0, rotate: 0 }}
							transition={{ duration: 0.6, delay: 1.2 + index * 0.06, type: 'spring', bounce: 0.6 }}
						>
							{letter}
						</motion.span>
					))}
